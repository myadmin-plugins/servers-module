...
namespace Detain\MyAdminServers\Tests\Support {
    /**
    ...
    class LogSpy
    {
         ...
         * @return void
         */
        public static function record($module, $level, $message, $id = '')
        {
            self::$calls[] = [
                'module' => $module,
                'level' => $level,
        ...
        /**
         * Recorded calls at one log level.
         *
         * @param string $level
         * @return array<int,array<string,mixed>>
         */
        public static function callsAtLevel($level)
        {
            $matches = array_filter(self::$calls, static function (array $call) use ($level) {
    ...
    /**
     * Records calls to the loose global helper functions the plugin reaches
     * for (chatNotify(), check_order_from(), setServerStatus(), ...).
     */
    class FunctionSpy
    {
        /**
         ...
         * @return void
         */
        public static function record($function, array $args = [])
        {
            if (!isset(self::$calls[$function])) {
                self::$calls[$function] = [];
            }
        ...
        /**
         * Every recorded argument list for one function, in order.
         *
         * @param string $function
         * @return array<int,array<int,mixed>>
         */
        public static function callsFor($function)
        {
            return isset(self::$calls[$function]) ? self::$calls[$function] : [];
        }
...
namespace MyAdmin {
    if (!class_exists(App::class, false)) {
        ...
        class App
        {
            /** @var \Detain\MyAdminServers\Tests\Support\HistorySpy|null */
            private static $history;

            /**
             * @return \Detain\MyAdminServers\Tests\Support\HistorySpy
             */
            public static function history()
            {
                if (self::$history === null) {
                    self::$history = new \Detain\MyAdminServers\Tests\Support\HistorySpy();
                }
                return self::$history;
            }
            ...
            /**
             * The plugin installer's real function_requirements() proxies
             * here. Test stubs are already declared, so report success.
             *
             * @param string $function
             * @return bool
             */
            ...
            {
                \Detain\MyAdminServers\Tests\Support\FunctionSpy::record('function_requirements', [$function]);
                return true;
            }
        }
    }
